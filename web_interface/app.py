...
def bank(bank_type):
    ...
    for column in ["open","high","low","close"]:
        df1=scaler.fit_transform(np.array(df[column]).reshape(-1,1))
        ...
        end=datetime.strptime("2024-01-30", '%Y-%m-%d').date()
        date=datetime.strptime('2024-01-01', '%Y-%m-%d').date()
        temp=df1[-30-pre_len:len(df1)-pre_len]
        ...
        while (date <= end):
            ...
            pre.append(model.predict(temp)[0])
            temp= temp.flatten()
        ...
        if (column=='open'):
            df = pd.concat([df, pd.DataFrame({'Date': dates})])
            predict=np.append(predict,pre)
            pre_len=len(pre)
        else:
            predict=predict[:-len(pre)]
            predict=np.append(predict,pre)
        df[f'predicted_{column}']=predict
    df=df.reset_index(drop=True)
    df.to_csv(f"{app.root_path}/temp/{bank_type}.csv")

    trace1 = go.Scatter(x=df["Date"], y=df["close"], mode='lines', name='Actual')
    trace2 = go.Scatter(x=df[len(df)-len(test_predict):]["Date"], y=df[len(df)-len(test_predict):][f"predicted_close"], mode='lines', name='Predicted')
...
@app.route('/<bank>/<type>', methods=['GET'])
def olhc(bank,type):
    print(bank,type)
    # date = request.form['date']
    # print(date)
    df=pd.read_csv(f"{app.root_path}/temp/{bank}.csv")

    trace1 = go.Scatter(x=df["Date"], y=df[type], mode='lines', name='Actual')
    trace2 = go.Scatter(x=df[len(df)-len(test_predict):]["Date"], y=df[len(df)-len(test_predict):][f"predicted_{type}"], mode='lines', name='Predicted')
    layout = go.Layout(title='Stock Price Prediction', xaxis=dict(title='Date'), yaxis=dict(title='Price'))
    fig = go.Figure(data=[trace1, trace2], layout=layout)
    # fig = px.line(df, x="Date", y=["predicted"], title="Stock Price Prediction")
    fig.update_layout(width=1000, height=600)
    try:
        ...
        print(str(e))
    return render_template('olhc.html',bank=bank.upper(),graphJSON=graphJSON,type='none',val=type)
...
def form(bank_type):
    ...
    trace1 = go.Scatter(x=df["Date"], y=df["close"], mode='lines', name='Actual')
    trace2 = go.Scatter(x=df[len(df)-len(test_predict):]["Date"], y=df[len(df)-len(test_predict):]["predicted_close"], mode='lines', name='Predicted')
    layout = go.Layout(title='Stock Price Prediction', xaxis=dict(title='Date'), yaxis=dict(title='Price'))
    fig = go.Figure(data=[trace1, trace2], layout=layout)
    # fig = px.line(df, x="Date", y=["predicted"], title="Stock Price Prediction")
    fig.update_layout(width=1000, height=600)
    ...
    if(date<=datetime.strptime("2024-12-30", '%Y-%m-%d').date()):
